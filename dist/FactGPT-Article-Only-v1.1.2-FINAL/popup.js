...
function resetRenderedResultState() {
  renderedResultSignature = "";
  renderedResultIdentity = "";
}

// Returns open detail rows only when a changed payload belongs to the same run.
// A new run, mode switch, error, or no-result state intentionally starts closed.
function decisionDetailsToRestore(mode, state, output) {
  const signature = resultRenderSignature(mode, state);
  if (!signature) {
    resetRenderedResultState();
    return null;
  }
  if (signature === renderedResultSignature) return null;

  const identity = resultRenderIdentity(mode, state);
  const openIds = identity === renderedResultIdentity
    ? captureOpenDecisionDetailIds(output)
    : [];
  renderedResultSignature = signature;
  renderedResultIdentity = identity;
  return openIds;
}
...
function renderAnalysisState(state, tabId) {
  ...
  setSectionVisible(ui.resultsSection, true);
  const openDecisionDetails = decisionDetailsToRestore(MODES.article, state, ui.factOpinionOutput);
  if (openDecisionDetails !== null) {
    ...
    restoreOpenDecisionDetailIds(ui.factOpinionOutput, openDecisionDetails);
...
function renderPodcastState(state) {
  ...
  setSectionVisible(ui.resultsSection, true);
  const openDecisionDetails = decisionDetailsToRestore(MODES.podcast, state, ui.factOpinionOutput);
  if (openDecisionDetails !== null) {
