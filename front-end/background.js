...
async function extractArticleText(url, tabId, key) {
  ...
  try {
    ...
    const data = await postBackend("/extract", { url: url.trim() });
  ...
  const rendered = await postBackend("/extract-rendered", { url: url.trim() });
...
async function runAnalysisJob({ key, url, tabId }) {
  ...
  try {
    ...
    const biasResult = await postBackend("/analyze-bias", {
      text: articleText,
    ...
    try {
      const researchResult = await postBackend("/research", {
        text: articleText,
        title: "Article Analysis"
      });
